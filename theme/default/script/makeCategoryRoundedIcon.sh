...
convert -size ${PYD_ICON_CATEGORY_WIDTH}x${PYD_ICON_CATEGORY_HEIGHT} canvas:${COLOUR1} /tmp/canvas1.png

# Cut out rounded rectangles in all 4 corners
convert /tmp/canvas1.png -alpha set  -compose Over -quality 100 \
  \( -size 20x20 xc:transparent -draw "circle 20,20 20,0" \
  ...
  /tmp/corner_cutoff.png
      
convert -size ${SMWIDTH2}x${SMHEIGHT2} -pointsize ${FONTSIZE} -background ${COLOUR2} \
    -fill ${COLOUR1} -gravity Center caption:"${PYD_CATEGORY_NAME}" /tmp/canvas3.png
